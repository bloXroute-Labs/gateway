...
type BxBlockConverter interface {
	BxBlockToBroadcast(*types.BxBlock, types.NetworkNum) (*bxmessage.Broadcast, types.ShortIDList, error)
...
func (bp *rlpBlockProcessor) ProcessBroadcast(broadcast *bxmessage.Broadcast) (*types.BxBlock, int, error) {
	...
	if err == ErrMissingShortIDs {
		log.Debugf("block %v from BDN is missing %v short IDs", broadcast.BlockHash(), len(missingShortIDs))
...
func (bp *rlpBlockProcessor) BxBlockToBroadcast(block *types.BxBlock, networkNum types.NetworkNum) (*bxmessage.Broadcast, types.ShortIDList, error) {
	bp.lock.Lock()
...
func (bp *rlpBlockProcessor) BxBlockFromBroadcast(broadcast *bxmessage.Broadcast) (*types.BxBlock, types.ShortIDList, error) {
	...
	bp.markProcessed(blockHash)
	block := types.NewRawBxBlock(broadcast.BlockHash(), rlpBlock.Header, txs, rlpBlock.Trailer, rlpBlock.TotalDifficulty, rlpBlock.Number)
