...
func newTestBxTxStore() BxTxStore {
	return newBxTxStore(&utils.MockClock{}, 30*time.Second, 30*time.Second, 10*time.Second,
		NewEmptyShortIDAssigner(), NewHashHistory("seenTxs", 30*time.Minute), nil, 30*time.Minute)
}
...
func TestBxTxStore_clean(t *testing.T) {
	...
	store := newBxTxStore(&clock, 30*time.Second, 30*time.Second, 10*time.Second, NewEmptyShortIDAssigner(), NewHashHistory("seenTxs", 30*time.Minute), cleanedShortIDsChan, 30*time.Minute)
...
func TestBxTxStore_clean256K(t *testing.T) {
	...
	store := newBxTxStore(&clock, 30*time.Second, 300*time.Hour, 10*time.Second, NewEmptyShortIDAssigner(), NewHashHistory("seenTxs", 30*time.Minute), nil, 30*time.Minute)
...
func TestHistory(t *testing.T) {
	...
	store := newBxTxStore(&clock, 30*time.Minute, 3*24*time.Hour, 10*time.Minute,
		NewEmptyShortIDAssigner(), newHashHistory("seenTxs", &clock, 30*time.Minute), cleanedShortIDsChan, 30*time.Minute)
...
func TestHistoryTxWithShortID(t *testing.T) {
	...
	cleanedShortIDsChan := make(chan types.ShortIDsByNetwork)
	store := newBxTxStore(&clock, 30*time.Second, 30*time.Second, 10*time.Second, NewEmptyShortIDAssigner(), newHashHistory("seenTxs", &clock, 60*time.Minute), cleanedShortIDsChan, 30*time.Minute)
	hash1 := types.SHA256Hash{1}
	content1 := types.TxContent{1}
...
func TestBxTxStore_ResetSeenTxTime(t *testing.T) {
	clock := utils.MockClock{}
	cleanedShortIDsChan := make(chan types.ShortIDsByNetwork)
	store := newBxTxStore(&clock, 30*time.Second, 30*time.Second, 10*time.Second, NewEmptyShortIDAssigner(), newHashHistory("seenTxs", &clock, 60*time.Minute), cleanedShortIDsChan, 30*time.Second)
