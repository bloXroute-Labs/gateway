...
type SDNRoutingConfig struct {
	...
	MinerTransactionReceptionDelay       int                  `json:"miner_transaction_reception_delay_ms"`
	TierTransactionDelay                 TierTransactionDelay `json:"tier_transaction_delay_ms"`
...
// TierTransactionDelay represents transaction delays based on account tier
type TierTransactionDelay struct {
	Propagation map[AccountTier]int `json:"propagation"`
	Reception   map[AccountTier]int `json:"reception"`
}
