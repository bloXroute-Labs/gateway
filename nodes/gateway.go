...
type gateway struct {
	...
	seenBlockConfirmation baseservices.HashHistory
...
func NewGateway(parent context.Context, bxConfig *config.Bx, bridge blockchain.Bridge, ws blockchain.WSProvider, blockchainPeers []types.NodeEndpoint) (Node, error) {
	...
	g := &gateway{
		...
		seenBlockConfirmation: baseservices.NewHashHistory("blockConfirmation", 30*time.Minute),
...
func (g *gateway) Run() error {
	...
	feedProvider := servers.NewFeedManager(g.context, g, g.feedChan, networkNum,
		g.wsProvider, accountModel, g.sdn.GetCustomerAccountModel,
		privateCertFile, privateKeyFile, *g.BxConfig)
...
func (g *gateway) handleBridgeMessages() error {
	...
	for {
		...
		case confirmBlock := <-g.bridge.ReceiveConfirmedBlockFromNode():
			...
			txList := make(types.SHA256HashList, 0, len(confirmBlock.Txs))
			for _, tx := range confirmBlock.Txs {
				txList = append(txList, tx.Hash())
			}

			bcnfMsg.Hashes = txList
			bcnfMsg.SetNetworkNum(g.sdn.NetworkNum())
			bcnfMsg.SetHash(confirmBlock.Hash())
			blockchainConnection := connections.NewBlockchainConn(g.blockchainPeers[0])
...
func (g gateway) handleMEVBundleMessage(mevBundle bxmessage.MEVBundle, source connections.Conn) {
	...
	if g.seenMEVMinerBundles.SetIfAbsent(mevBundle.Hash().String(), time.Minute*30) {
		...
		if source.Info().IsRelay() {
			...
			source.Log().Tracef("%v mevBundle message hash %v to mev relay, duration: %v", event, mevBundle.Hash(), time.Since(start).Milliseconds())
			return
		}
...
func (g gateway) handleMEVSearcherMessage(mevSearcher bxmessage.MEVSearcher, source connections.Conn) {
	...
	if g.seenMEVSearchers.SetIfAbsent(mevSearcher.Hash().String(), time.Minute*30) {
		...
		if source.Info().IsRelay() {
			...
			source.Log().Tracef("%v mevSearcher message hash %v to mev relay, duration: %v", event, mevSearcher.Hash(), time.Since(start).Milliseconds())
			return
		}
	...
	source.Log().Tracef("%v mevSearcher message msg: %v in network %v to relays, result: [%v], durarion: %v", event, mevSearcher.Hash(), mevSearcher.GetNetworkNum(), broadcastRes, time.Since(start).Milliseconds())
}
