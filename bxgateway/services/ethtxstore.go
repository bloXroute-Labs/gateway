...
const cleanNonceInterval = 10 * time.Second
...
func NewEthTxStore(clock utils.Clock, cleanupInterval time.Duration, maxTxAge time.Duration,
	...
	return &EthTxStore{
		BxTxStore:    newBxTxStore(clock, cleanupInterval, maxTxAge, noSIDAge, assigner, hashHistory, cleanedShortIDsChannel),
...
func (t *EthTxStore) Add(hash types.SHA256Hash, content types.TxContent, shortID types.ShortID,
	...
	if !validate && !t.isReuseNonceActive(network) {
		return result
	}
	...
	// upon first seeing tx, validate that it's an Ethereum transaction
	blockchainTx, err := result.Transaction.BlockchainTransaction()
	if err != nil {
	...
	if networkChainID != 0 && txChainID != 0 && networkChainID != txChainID {
		...
		t.BxTxStore.RemoveHashes(&hashToRemove, NoReEntryProtection, "chainID mismatch")
		result.FailedValidation = true
		return result
...
func (nt nonceTracker) isReuseNonceActive(networkNum types.NetworkNum) bool {
	return nt.networkConfig[networkNum].EnableCheckSenderNonce
}
...
func (nt *nonceTracker) track(tx *types.EthTransaction, network types.NetworkNum) (bool, *types.SHA256Hash) {
	if !nt.isReuseNonceActive(network) {
		return false, nil
	}
