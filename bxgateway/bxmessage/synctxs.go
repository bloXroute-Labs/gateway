...
func (m *SyncTxsMessage) size() uint32 {
	return uint32(HeaderLen + types.NetworkNumLen + types.UInt32Len + ControlByteLen + m._size)
}
...
func (m *SyncTxsMessage) Unpack(buf []byte, _ Protocol) error {
	...
	return m.unpackContentShortIds(&buf, HeaderLen+types.NetworkNumLen+types.UInt32Len, txCount)
}
