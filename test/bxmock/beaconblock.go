...
// NewBellatrixBeaconBlock creates beacon block using execution layer block
func NewBellatrixBeaconBlock(t *testing.T, slot int, prevBlock interfaces.ReadOnlySignedBeaconBlock, ethBlock *ethtypes.Block) interfaces.ReadOnlySignedBeaconBlock {
...
// NewCapellaBeaconBlock creates beacon block using execution layer block
func NewCapellaBeaconBlock(t *testing.T, slot int, prevBlock interfaces.ReadOnlySignedBeaconBlock, ethBlock *ethtypes.Block) interfaces.ReadOnlySignedBeaconBlock {
	ezDecode := func(t *testing.T, str string) []byte {
		b, err := hex.DecodeString(strings.TrimPrefix(str, "0x"))
		assert.NoError(t, err)
