...
func TestTxs_Fixture(t *testing.T) {
	expectedHash1, _ := types.NewSHA256HashFromString(fixtues.TxsHash1)
	expectedContent1, _ := hex.DecodeString(fixtues.TxsContent1)
	expectedHash2, _ := types.NewSHA256HashFromString(fixtues.TxsHash2)
	expectedContent2, _ := hex.DecodeString(fixtues.TxsContent2)

	b, _ := hex.DecodeString(fixtues.TxsMessage)
	...
	assert.Equal(t, types.ShortID(fixtues.TxsShortID1), items[0].ShortID)
	assert.Equal(t, expectedHash2, items[1].Hash)
	assert.Equal(t, types.TxContent(expectedContent2), items[1].Content)
	assert.Equal(t, types.ShortID(fixtues.TxsShortID2), items[1].ShortID)
